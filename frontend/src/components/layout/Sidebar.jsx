...
export default function Sidebar({ 
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full max-h-screen bg-sidebar-bg text-sidebar-text overflow-hidden transition-all duration-300 ease-in-out select-none">
      ...
      <div className={`flex items-center border-b border-sidebar-border flex-shrink-0 transition-all duration-300 ${
        isCollapsed ? 'px-2 py-3 justify-center' : 'px-3 py-3 justify-between'
      }`}>
        {!isCollapsed ? (
      ...
      {!isCollapsed && (
        <div className="px-3 pt-3 flex-shrink-0">
          <button 
            ...
            className="w-full flex items-center justify-between px-3 py-2 text-xs rounded-lg border border-sidebar-border bg-sidebar-surface/50 text-sidebar-text-muted hover:text-sidebar-text hover:bg-sidebar-hover transition-all duration-150 group"
          >
            <span className="flex items-center gap-2 font-medium">
              <Search className="w-3.5 h-3.5 text-sidebar-text-muted group-hover:text-sidebar-text" />
              Quick search...
            </span>
            <kbd className="text-[9px] font-mono font-bold px-1.5 py-0.5 rounded bg-sidebar-surface border border-sidebar-border text-sidebar-text-muted">⌘K</kbd>
          </button>
        </div>
      )}
      ...
      <nav className={`flex-1 overflow-y-auto overflow-x-hidden scrollbar-none transition-all duration-300 ${
        isCollapsed ? 'px-2 py-4 flex flex-col gap-3' : 'px-3 py-4 flex flex-col gap-5'
        ...
        {!isCollapsed && (
          <div className="mt-auto px-1 pt-3 flex-shrink-0">
            <div className="rounded-xl border border-sidebar-border bg-sidebar-surface/40 p-3.5 space-y-2.5 transition-all duration-200">
  ...
  return (
    <>
      ...
      <aside className={`hidden lg:block h-screen max-h-screen border-r border-sidebar-border bg-sidebar-bg sticky top-0 left-0 flex-shrink-0 z-30 overflow-hidden transition-all duration-300 ease-in-out ${
      ...
      >
        ...
        />
        <aside 
          className={`absolute top-0 bottom-0 left-0 w-60 max-w-[80vw] border-r border-sidebar-border bg-sidebar-bg overflow-hidden transition-transform duration-250 ease-in-out transform ${
